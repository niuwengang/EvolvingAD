 ...
 * @return
 */
bool PoseGraph::UpdatePose(const PoseMsg &gnss_msg, const PoseMsg &lidar_odom_msg, PoseMsg &fusion_odom_msg)
{
    /*1--build fusion odom use lidar odom(has been transformed to gnss coor)*/
    fusion_odom_msg_.pose = lidar_odom_msg.pose;
    fusion_odom_msg_.time_stamp = lidar_odom_msg.time_stamp;

    if (CheckNewKeyFrame() == true)
    {
        AddVertexandEdge(gnss_msg);

        if (graph_optimizer_ptr_->Opimtize() == true)
        {
            graph_optimizer_ptr_->GetOptPoseQueue(opted_pose_queue_);
            fusion_odom_msg.pose = opted_pose_queue_.back(); // opted
        }
    }
    return true;
}
 ...
 * @return
 */
bool PoseGraph::CheckNewKeyFrame()
{

    static Eigen::Matrix4f last_keyframe_pose = fusion_odom_msg_.pose;

    bool has_new_keyframe_flag = false;
    /*[1]--check if keyframe*/
    if (keyframe_msg_queue_.size() == 0)
    {
        has_new_keyframe_flag = true;
        last_keyframe_pose = fusion_odom_msg_.pose;
    }
    if (fabs(fusion_odom_msg_.pose(0, 3) - last_keyframe_pose(0, 3)) +
            fabs(fusion_odom_msg_.pose(1, 3) - last_keyframe_pose(1, 3)) +
            fabs(fusion_odom_msg_.pose(2, 3) - last_keyframe_pose(2, 3)) >
        2.0) // todo add params
    {
        last_keyframe_pose = fusion_odom_msg_.pose;
        has_new_keyframe_flag = true;
    ...
    if (has_new_keyframe_flag == true)
    {
        ...
        keyframe_msg.time_stamp = fusion_odom_msg_.time_stamp;
        keyframe_msg.pose = fusion_odom_msg_.pose;
...
bool PoseGraph::AddVertexandEdge(const PoseMsg &gnss_odom_msg)
{
    ...
    if (graph_optimizer_ptr_->GetOptNodeNum() == 0) // first need fixed
    {
    ...
    if (node_num >= 2)
    {
        ...
        vec_noise << 0.5, 0.5, 0.5, 0.001, 0.001, 0.001;
